...
/*
  Module Operations: 
  ==================
  In this package I added 3 functions:
  extractAtbs(); which returns a vector of attribute pairs
  extractComment(); returns string of comment text
  extractText(); returns string of the text
  These functions are used for XmlDocument's parsing and storing which make it easier.
  ...
  std::vector<std::pair<std::string, std::string>> extractAtbs();				//returns a vector of attribute pairs
  std::string extractComment();				//returns string of comment text
  std::string extractText();				//returns string of the text
  ...
  ====================
  my ver 1.0
  added three functions to the original xmlElementParts.h file
  std::vector<std::pair<std::string, std::string>> extractAtbs();
...
public:
  ...
  std::vector<std::pair<std::string, std::string>> extractAtbs();   //my function to extract attributes from this xml part
  std::string extractComment();  //my function to extract comments
  std::string extractText(); //my function to extract text from text element 
  void toLower();
  void trimFront();
  void clear();
  std::string show();
...
inline int XmlParts::length() { return (int)toks.size(); } //how many toks
